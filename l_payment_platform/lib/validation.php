...
class validation 
{
    ...
    function hasMetCondition($request,$key,$rule_to_validate,$alias)
    {
        $val = $request[$key];
    ...
    function numericComparism($request,$key,$rule_to_validate,$alias)
    {
        $val = $request[$key];
        ...
        if($r_rule[0] == 'contains')
        {
            if (strpos($val, $r_rule[1]) == false) {
                $this->error = true;
                $this->messageBag[] = $alias.' field must contain "'.$r_rule[1].'" character.';
                return $this->error;
            }
...
//            echo $request[$r_rule[1]]."~".$val;
            ...
            {
                $this->error = true;
                $this->messageBag[] = $alias.' fields do not match.';
                return $this->error;
            }
        }
        // if($r_rule[0] == 'unique')
        ...
        //     {
        //         $this->error = true;
        //         $this->messageBag[] = $alias.' already exists ';
        //         return $this->error;
        //     }
        // }
    }
